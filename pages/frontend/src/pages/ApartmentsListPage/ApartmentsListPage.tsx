...
const ApartmentsListPage = ({apartments, setApartments, isMock, setIsMock}:Props) => {
    ...
    const {apartment_name} = useAppSelector((state) => state.apartments)
    ...
    const fetchApartments = async () => {
        try {
            const response = await fetch(`http://localhost:8000/api/apartments/?apartment_name=${apartment_name.toLowerCase()}`)
