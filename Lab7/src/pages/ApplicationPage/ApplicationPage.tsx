...
const ApplicationPage = () => {
	...
	const [total_price, setTotal_price] = useState<string>(
	...
	return (
		<Form onSubmit={sendApplication} className='pb-5'>
			<h2 className='mb-5'>
				{isDraft ? 'Черновое заявление' : `Заявление №${id}`}
			</h2>
			<Row className='mb-5 fs-5 w-25'>
				<CustomDatePicker
					label='Введите дату приезда'
					value={start_date}
					setValue={setStart_date}
					disabled={!isDraft || is_superuser}
				/>
				<CustomDatePicker
					label='Введите дату выезда'
					value={final_date}
					setValue={setFinal_date}
					disabled={!isDraft || is_superuser}
				/>
				{isCompleted && (
					<CustomInput
			...
			<Row>
				{application.apartments.length > 0 ? (
					application.apartments.map((apartment: T_Apartment) => (
						<Row
							key={apartment.id}
			...
			{isDraft && !is_superuser && (
				<Row className='mt-5'>
					<Col className='d-flex gap-3 justify-content-center'>
						<Button color='success' onClick={saveApplication}>
							Сохранить
						</Button>
						<Button color='primary' type='submit'>
							Отправить
						</Button>
						<Button color='danger' onClick={deleteApplication}>
							Удалить
						</Button>
					</Col>
				</Row>
